...
public class RayDetector : MonoBehaviour
{
    ...
    [SerializeField] public Transform toStart;
    [SerializeField] public GameObject toCompare; // Player or any object that requires LOS detection
    [Header("Ray Distance")]
    ...
    //{
    //    if (Physics.SphereCastAll(toStart.position, sphereCastRadius, toStart.transform.forward, out RaycastHit hit, distance, toIgnore)) // Detected Something
    //    {
    //        Debug.Log(hit.transform.gameObject.name);
    //        if (hit.transform.gameObject == toCompare)
    //        {
